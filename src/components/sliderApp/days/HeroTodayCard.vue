<template>
  <div class="card_container">
    ...
    <div class="card__body">
      <ul>
      <li class="card__element" v-for="(text, index) in getDateObj.text" :key="index">
        <div class="card__element_text">{{ text }}</div><button class="card__element_button" @click="deleteText(index)"></button>
      </li>

    </ul>
      <input placeholder="+ Добавить заметку" class="card__body_input" type="text" v-model="newText" @blur="addText" @keyup.enter="addText" />
    </div>
    <div class="momo"></div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    getDateObj() {
      return this.daysDATA[this.todayIndex + this.daysNumbers.Чт.count]; // получить обьект дня
    },
    getFullDate() {
      return (
    ...
    }, // получить полную дату исходя из обьекта
    getDay() {
      return this.daysDATA[this.todayIndex + this.daysNumbers.Чт.count].day;
    }, // получить день
  },
  methods: {
    deleteText(index) {
